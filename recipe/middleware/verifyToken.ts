...
import { AuthorizationResponseType, ResponseBodyType } from "../types/response"
...
const handleJWTAuth = async (tokenRaw: String): Promise<ResponseBodyType> => {
	try {
		...
		const getProfileExistsQuery: DBQueryType = {
			text: "SELECT * FROM profiles WHERE id = $1",
			values: [payload["id"].toString()]
		}

		//? Get profile by the _id from the payload
		const existingProfileResponse: ResponseBodyType =
			await ProfileQuery.get(getProfileExistsQuery, true)
